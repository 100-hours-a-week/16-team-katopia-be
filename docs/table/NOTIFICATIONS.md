...
- actor 정보는 스냅샷으로 저장해 닉네임/이미지 변경 영향을 최소화한다.
- ref_id로 대상 리소스(게시글/투표 등)를 연결한다.
- 수신자 기준 최신순 정렬/페이징을 고려한다.
- ref_id는 다수 테이블의 FK를 담지만 타입은 애플리케이션 계층에서 분기해 해석한다.

### 알림 유형(NotificationType)
유형 | 의미
---|---
FOLLOW | 다른 사용자가 나를 팔로우
POST_CREATED | 내가 팔로우한 사용자가 게시글을 작성
...
2 | recipient_id | BIGINT | nn | 수신자
3 | actor_id | BIGINT |  | 행위자(시스템 알림은 null)
...
| 인덱스/제약 | 근거 |
| --- | --- |
| `idx_notifications_recipient_created (recipient_id, created_at)` | 수신자별 알림함 최신순 조회와 커서 페이징을 빠르게 처리하기 위한 인덱스 |

## 5. 운영 정책
- 생성일 기준 30일 경과 시 읽음 여부와 무관하게 삭제한다.
- 알림 내용 보존을 위해 스냅샷(actor 닉네임/이미지)을 저장한다.
- 시스템 알림은 `actor_id = null`, `actor_nickname_snapshot = SYSTEM`으로 저장한다.
- 알림 이미지 스냅샷은 유형별로 다르게 채운다(FOLLOW: actor 프로필, POST_CREATED/POST_LIKE/POST_COMMENT: 게시글 첫 이미지, VOTE_CLOSED: 투표 첫 이미지).
