...
#### MongoDB 도입 근거
- 샤딩 기반 수평 확장이 MySQL 파티셔닝/샤딩 대비 단순하다.
- 채팅 메시지의 스키마 확장(첨부/리액션/메타데이터)이 잦을 수 있다.
- 쓰기 성능 자체는 환경/설정에 따라 다르지만, 샤딩으로 **쓰기 부하 분산**이 쉽다.
- 복잡한 조인/강한 트랜잭션 요구가 낮아 문서형 저장소가 적합하다.

#### MongoDB 버전
- **MongoDB 7.0 LTS** (운영 안정성과 장기 지원 우선)
  - 실제 도입 시점의 클라우드/사내 정책 LTS 버전과 정합성 확인 필요
...
- 그룹 채팅 지원(방 단위 브로드캐스트)
- 대량 채팅에서 저지연 목표
- 채널 내 순서 우선(전송/저장 분리 시 순서 기준 명시)

#### STOMP vs Raw WebSocket 비교

| 항목 | STOMP(WebSocket) | Raw WebSocket |
| --- | --- | --- |
| 라우팅 | 구독/목적지 표준 제공 | 직접 구현 필요 |
| 인증/권한 | CONNECT/구독 인터셉터로 일관 처리 | 프레임 파싱/세션 관리 직접 구현 |
| 운영 난이도 | 낮음(프레임/에러 규격 존재) | 높음(프로토콜/에러 규격 정의 필요) |
| 지연/오버헤드 | 약간의 프레임 오버헤드 | 최소 오버헤드(직접 최적화) |
| 확장/유지보수 | 표준 기반 확장 용이 | 팀 규격 의존, 유지보수 비용↑ |
---
